 ...
 */
static int
desfire_transceive(MifareTag tag, const uint8_t *msg, size_t msg_len, uint8_t *res, size_t res_size, size_t *res_len)
{
    ...
    if (tag->device == NULL) {
      ...
      DWORD __pcsc_recv_len = sizeof(res_buf);
      if ((SCARD_S_SUCCESS != SCardTransmit(tag->hCard, SCARD_PCI_T1, msg_buf, len, &__pcsc_rcv_pci, res_buf, &__pcsc_recv_len)) < 0) {
        return errno = EIO, -1;
      }
      rc = __pcsc_recv_len;
    } else {
      if ((rc = nfc_initiator_transceive_bytes (tag->device, msg_buf, len, res_buf, sizeof(res_buf), 0)) < 2) {
